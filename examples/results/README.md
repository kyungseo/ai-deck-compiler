...
같은 발표 내용을 preset/theme별 blueprint와 PPTX로 재생성할 수 있습니다.

![Showcase gallery](showcase-gallery.png)

포함 기준:

- `showcase-teal-dark.blueprint.yaml` / `showcase-teal-dark.pptx` — 신규 deck 기본 추천 경험 (`teal + dark`)
- `showcase-vivid-dark.blueprint.yaml` / `showcase-vivid-dark.pptx` — product demo와 feature showcase 톤 (`vivid + dark`)
- `showcase-modern-light.blueprint.yaml` / `showcase-modern-light.pptx` — light/business 공유 문서 톤 (`modern + light`)
