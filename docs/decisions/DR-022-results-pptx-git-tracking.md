# DR-022: examples/results/*.pptx git 추적 정책
...
preset별 대표 PPTX 결과물을 git으로 추적할 것인가, gitignore로 제외할 것인가?

## Decision

`examples/results/*.pptx`를 git으로 추적한다.
...
| examples/results/*.pptx만 추적 (채택) | GitHub에서 즉시 다운로드 가능, 첫인상 강화 | 렌더러 변경 시 수동 재생성 필요, binary diff |
