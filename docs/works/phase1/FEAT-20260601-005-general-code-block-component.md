...
- [ ] 본문형 slide에서 code block 표현 방식이 결정되어 Work Discovery에 기록된다.
- [ ] `content`, `two-column`, `appendix` 중 합의된 1차 대상에 boxed monospace code block이 적용된다.
- [ ] 기존 `appendix` code block 동작이 회귀하지 않는다.
- [ ] README / USER-MANUAL / SYSTEM-MANUAL이 실제 지원 범위와 일치한다.
- [ ] syntax highlighting을 이번 Work에 포함할지, 후속으로 둘지 결정이 기록된다.
- [ ] `npm run typecheck` 통과.
- [ ] `npm test` 통과.
- [ ] 관련 sample 또는 showcase preview로 visual QA가 수행된다.
