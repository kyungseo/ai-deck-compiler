...
function findInPath(commands: string[]): string | null {
...
function findAbsolute(paths: string[]): string | null {
...
function findPdftoppm(): string | null {
  ...
  if (os === 'win32') {
    // Common locations from poppler-windows releases
    return findAbsolute([
